Spree::ReimbursementType::ReimbursementHelpers.module_eval do
  def create_creditable_with_store_credits_decoration(reimbursement, unpaid_amount)
    ...
    Spree::StoreCredit.new(user: reimbursement.order.user, amount: unpaid_amount, category: category, memo: "Refund for order #{reimbursement.order.number}", currency: reimbursement.order.currency)
  end
